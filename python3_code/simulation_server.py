...
parser.add_argument('--IV', type = int,nargs = '+',
...
if type_of_network == 'single fully connected agent on torus' or type_of_network == 'single fast communicator agent on torus' or "torus_w_error": #upodate with torus error
    single_agent = int(round(random()*100)) # 0 through 99, inclusive
else: 
...
def one_round(current_round):
    for i in range(population): #runs through population
        ...
        if (current_round%info_velocity == 0 or current_round == 0): # update with single no error and torus
            if(i==single_agent or type_of_network != 'torus_w_error'):
                agents[i].compare_with_neighbors(neighbors)
            elif(type_of_network == 'torus_w_error'):
                agents[i].compare_with_neighbors_with_error(neighbors)
            if agents[i].should_agent_explore(): 
                explore_space(*agents[i].current_agent_values())
        else:
            explore_space(*agents[i].current_agent_values())    
...
for iv_values in range(len(args.IV)):
    
    info_velocity = args.IV[iv_values] #ex - 1 (every turn), 2 (every two turns, communicatioon allowed), 5 (every fifth turn, communication allowed), or 10
    ...
    if type_of_problem_space == "NK":
        ...
        NK.to_csv("NK_"+type_of_network+"_V"+str(info_velocity)+".csv",index=False)
    if type_of_problem_space == "TSP":
        #TSP_final_list=[]
        #for i in range(len(final_round)):
        ...
        TSP.to_csv("TSP_"+type_of_network+"_V"+str(info_velocity)+".csv",index=False)
    if type_of_network == 'single fully connected agent on torus' or type_of_network == 'single fast communicator agent on torus':
        ...
        if type_of_problem_space == "NK":
            ...
            Single_final.to_csv("Singleagent_NK"+type_of_network+"_V"+str(info_velocity)+".csv",index=False)
        if type_of_problem_space == "TSP":
            ...
            single_final_list = [[a,b,c] for a,b,c in zip(single_agent_final_round, single_agent_final_score,single_agent_final_score_sem) ]
            Single_final=pd.DataFrame(data=single_final_list,columns=["round","final Score","SEM"])
            Single_final.to_csv("Singleagent_TSP"+type_of_network+"_V"+str(info_velocity)+".csv",index=False)
